...
def avg_terrain(terrain):
    # for times in range(3):
    for i in range(len(terrain) - 1):

        media = avg(terrain[i], terrain[i + 1])

        terrain[i] = media
        terrain[i + 1] = media

heights = get_heights(5, max_height)
stone_height = get_heights(0, stone_max_height)

avg_terrain(heights)
avg_terrain(stone_height)
